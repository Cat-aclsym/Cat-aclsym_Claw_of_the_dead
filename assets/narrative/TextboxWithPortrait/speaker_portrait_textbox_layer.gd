...
func _ready() -> void:
	var portrait_container = get_node_or_null("Anchor/Panel/HBox/PortraitPanel/PortraitBackgroundColor/DialogicNode_PortraitContainer")
	if portrait_container == null:
		push_warning("PortraitContainer introuvable dans la scène !")
		return

	var positions := ["left", "center", "right"]

	for pos in positions:
		if not portrait_container.has_node(pos):
			var container := Control.new()
			container.name = pos
			container.size = Vector2(200, 300)
			container.set_anchors_preset(Control.PRESET_CENTER)
			match pos:
				"left": container.position = Vector2(-300, 0)
				"right": container.position = Vector2(300, 0)
				"center": container.position = Vector2(0, 0)
			portrait_container.add_child(container)
